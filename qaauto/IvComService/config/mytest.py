...
def configure():
    ...
    if 'vkhost' in hosts[env]:
        ...
        if env == "QAE":
            ...
            channel_ppejpcea = "engine-PPEJPCEA"
            config['channels'][channel_ppejpcea] = {
                'connect-to': [
                    {
                        'host': 'd153617-001.dc.gs.com',
                        'port': 18000,

                        },
                ]
            }
            config['client-request-managers']['engine-PPEJPCEA-requestResponse'] = {
                'applicationid': 'MyIvComPy-rf',
                'auto-indicate-availability-load': 100,
                'channel': channel_ppejpcea,
                'request-failure-interval': 300,
            }
            ### PPSJPCEA
            channel_ppsjpcea = "engine-PPSJPCEA"
            config['channels'][channel_ppsjpcea] = {
                'connect-to': [
                    {
                        'host': 'd153617-002.dc.gs.com',
                        'port': 19000,
                        },
                ]
            }
            config['client-request-managers']['engine-PPSJPCEA-requestResponse'] = {
                'applicationid': 'MyIvComPy-rf',
                'auto-indicate-availability-load': 100,
                'channel': channel_ppsjpcea,
                'request-failure-interval': 300,
            }

        if env == 'PPE':
            ## configure HK viking
            channel_xpxz = "oma_routing"
            config['channels'][channel_xpxz] = {
                'accept-on': [
                    {
                         'port': 10311,
                         "heartbeat-request-interval":"8",
                         "maximum-in-process-send-buffer-size":"15MB",
                         "maximum-inactivity-interval":"16",
                         "maximum-messages-per-read":"3",
                         "maximum-read-block-size":"30KB",
                         "maximum-receive-buffer-size":"1MB",
                         "maximum-received-message-size":"10MB",
                         "maximum-send-buffer-size":"1MB",
                         "message-read-policy":"",
                         "minimum-read-block-size":"300B",
                        },
                ],
                 "connecter-accepter":"false",
            }

            # configure server datastream
            config['server-datastreams']['PPEAUCEA->ppe_XPXz'] = {
                    'applicationId': "IvComPy",
                    "auto-indicate-availability-load":"100",
                    "cache-enqueued-messages":"false",
                    "catalog":
                    [
                        "RejectOrderCommand",
                        "AcceptOrderCommand",
