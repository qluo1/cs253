...
def check_halfTick(price):
    """ test price is halfTick.

    >>> check_halfTick(2.13)
    False
    >>> check_halfTick(2.135)
    True
    >>> check_halfTick(1.13)
    False
    >>> check_halfTick(1.131)
    False
    >>> check_halfTick(1.135)
    False

    >>> check_halfTick(0.11)
    False

    >>> check_halfTick(0.115)
    False
    >>> check_halfTick(0.116)
    False
    >>> check_halfTick(0.1125)
    True

    >>> check_halfTick(0.097)
    False
    >>> check_halfTick(0.0975)
    True

    >>> check_halfTick(0.011)
    False
    >>> check_halfTick(0.012)
    False
    >>> check_halfTick(0.0125)
    True
...
def check_irrugular_tick(price):
    """  test irrugular tick price

    >>> check_irrugular_tick(0.011)
    False
    >>> check_irrugular_tick(0.0112)
    True

    >>> check_irrugular_tick(0.25)
    False
    >>> check_irrugular_tick(0.255)
    False
    >>> check_irrugular_tick(0.2525)
    False
    >>> check_irrugular_tick(0.2526)
    True

    """
    if not check_tick(price) and not check_halfTick(price):
        return True
    return False
...
def round_price(price,**kw):
    ...
    assert check_irrugular_tick(price) == True
