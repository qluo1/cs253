""" original adopted from  http://www.dalkescientific.com/Python
OrderedMultiDict has been modified to handle NVP list.
author: luosam@gs.com
"""
...
class _BaseMultiDict:
    ...
    def __getitem__(self, key):
        ...
        try:
            ...
            log.warn("unexpected key: %s, %s" % (key,e))
            return None
...
class OrderedMultiDict(_BaseMultiDict):
    ...
    Acts like a standard dictionary with the following features:
       ...
       - convert an existing NVP object toList, for crossing network.
       - construct new NVP object from nvp list in constructor.
    ...
    def __init__(self, multidict = None):
        ...
        if multidict is not None:
            ...
            ## recursive parsing nvp list into muiltidict
            for k, v in multidict:
                if isinstance(v,list):
                    try:
                        self[k] = OrderedMultiDict(v)
                    except ValueError,e:
                        self[k] = v
                else:
                    self[k] = v
    ...
    ### author:  luosam
    def toFormattedString(self,**kw):
        """ to formattedString. internal recursively call itself. """
        ## initial 
        buffer = ""
        ...
        for k,v in items:
            ## skip items
            if k in skips: continue
            if isinstance(v,OrderedMultiDict):
                #import ipdb;ipdb.set_trace()
                buffer += "%s%s:\n" % ("\t" * level, k)
                buffer += self.toFormattedString(items=v,level=level+1,skips=skips)
            else:
                #print "\t" * level, k,v
                #pprint("%s: %s" %(k,v),indent=level)
                buffer += "%s%s: %s\n" % ("\t" * level,k,v)
    ...
    def toList(self, **kw):
        ...
        for item in _items:
            ...
            assert len(item) == 2
            if isinstance(item[1],OrderedMultiDict):
                out.append((item[0],self.toList(items=item[1].allitems())))
            else:
                out.append(item)
        return out
