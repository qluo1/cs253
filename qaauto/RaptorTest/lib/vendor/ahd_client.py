...
class App():
    ...
    def __init__(self, cfgFile, logPath):

        print '\nArrowhead Client\nCopyright Fusion Systems 2013\n'

        if cfgFile[len(cfgFile)-3:] == '.py':
            cfgModule = cfgFile[:len(cfgFile)-3]
        else:
            cfgModule = cfgFile

        exec 'from %s import *' % cfgModule

        print 'Loaded configuration from %s:' % cfgModule
        pprint.pprint(CFG)
        ...
        self.fileLogger = logging.FileHandler(filename = logPath + '/%s-%s.log' %
...
def getApp(cfgFile='client_cfg.py', logPath='./log'):
    ...
    if not App.__instance__:
        App.__instance__ = App(cfgFile, logPath)
