...
class App():
    ...
    def __init__(self, cfg, logPath, cb_notify):
        ...
        #import pdb;pdb.set_trace()
        ## move old file to archive log
        if os.path.exists(logPath + '/%s.log' % self.logName):
            i = 2
            while os.path.exists(logPath + '/%s-%d.log' % (self.logName, i)):
                i += 1
            os.rename(logPath + '/%s.log' % self.logName,logPath + '/%s-%d.log' % (self.logName, i))
        ...
        print '\nLogging will be written to ' + logPath + '/%s.log' % self.logName
    ...
    def start(self, loadSeqNoFile=None):
        ...
        for vsNum in self.vsNums :
            ...
            self.socket[vsNum].setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1 )
            self.gotConnected[vsNum] = False
    ...
    def sendRequest(self, r, vsNum):
        ...
        # Serialize the object before send it back.
        return str(r)
    ...
    def showPacket2(self, p, type):
        if type == 'ESPCommon':
            p = ESPCommon(p)
        elif type == 'OrderCommon':
            p = OrderCommon(p)
        else:
            raise Exception('Unknown type: %s' % type)
        return self.__showPacket2(p)
    ...
    def testReport(self):
        ...
        if os.path.exists(self.logPath + '/results-%s.csv' %
                          self.logName ):
            self.info ('Start testReport: path exists')
            i = 2
            while os.path.exists(self.logPath + '/results-%s-%d.csv' %
                                 ( self.logName, i) ) :
                i += 1
            os.rename(self.logPath + '/results-%s.csv' %
                      (self.logName ),
                      self.logPath + '/results-%s-%d.csv' %
                      (self.logName, i ))

        f = open(self.logPath + '/results-%s.csv' %
                 (self.logName), 'w' )
        self.info('Writing test report to ' + self.logPath + '/results-%s.csv' %
                    self.logName )
