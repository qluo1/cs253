""" om2 related project configuration

TODO: 1) change code_base to OM2
      2) specify build output filder for all shared objects.
...
##### luosam
## om2 codebase
CODE_BASE   = "/local/data/home/eqtdata/sandbox/luosam/gitworks/om2"
## all build output
...
CXXFLAG_GS = "-pipe -DUSE_GNS -m64 -D__64BIT__ -DSYB_LP64 -D__LINUX -D__LINUX -D_64BIT_RHEL5_OR_GREATE -D__INOTIFY=1 \
              -DBOOST_FILESYSTEM_VERSION=3 -ffor-scope -Wall -DMETRICS_SUPPORTED"

## OM2/Ivcom build missing depency, so don't use it.
##OM2_IVCOM_BUILD = os.path.join(CODE_BASE,"ivcom","src","linux6_64_DBG")
## 
LIBPATHS  = [LIB_DIR, CPPUNIT_LIB,UUID_LIB,GSAUTH_LIB,JSON_LIB,PYTHON_LIB,BOOST_LIB,MONGO_LIB,GSKRB_LIB,LIBEV_LIB]
