...
class Worker(object):
    ...
    def __init__(self,work_queue,settings):
        ...
        assert isinstance(work_queue,Queue.PriorityQueue)
        ...
        self.server_ = zerorpc.Server(methods={
                                        settings.SERVICE_API_ORDER: self.order_request,
                                        settings.SERVICE_API_STATUS: self.session_status,
                                       })
        log.info("worker binding: %s" % settings.ORDER_API_ENDPOINT)
        self.server_.bind(settings.ORDER_API_ENDPOINT)
    ...
    def run(self):
        ...
        log.info("publisher started at: %s." % self.settings_.PUBSUB_ENDPOINT)
        _publisher = zerorpc.Publisher()
        _publisher.bind(self.settings_.PUBSUB_ENDPOINT)
        ## reverse lookup
        ...
        while self.running_:
            ...
            try:
                ...
                ## send to zerorpc
                method = getattr(_publisher,self.settings_.PUBSUB_API_NAME)
                method(order)
                log.debug("published order to name: %s" % self.settings_.PUBSUB_API_NAME)
            except Queue.Empty:
                gevent.sleep(0)
            except Exception,e:
                # handle error
