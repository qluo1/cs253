...
from utils import clientOrderId, IvComDictHelper
XREFS = ('X',)
...
class VikingOrder:
    ...
    tracker_ = VikingOrderTracker(settings.VIKING_SUB_ENDPOINT)

    api_ = zerorpc.Client(settings.VIKING_ORDER_ENDPOINT,
                            heartbeat=settings.HEARTBEAT_TIMEOUT,
                            timeout=settings.HEARTBEAT_TIMEOUT * 3)
    ...
    def __init__(self,**kw):
        ...
        if kw:
            if "exch" in kw:
                ...
                kw.pop("exch")
            assert self._data["vikingMarketId"] in ("SYDE","CHIA","HKGE")
            if "tif" in kw:
        ...
        today = datetime.now()
        self._data['orderId'] = "%s%s%s" % (today.strftime("%Y%m%d"),
                                            self._xref,clientOrderId("Viking_%s" % self._xref ,
                                            prefix=False,compact=False))
        ## default value
    ...
    def _flowAU(self):
        ...
        flowSpecAU.set("iosAccountExchangeCrossReference","%s/%s" % (self._xref,self._data["orderId"][9:]))
    ...
    def new(self,**kw):
        ...
        if self._data["vikingMarketId"] == "HKGE":
            order.set("executionCurrency","HKD")
            order.set("vikingMarketIdType","PrimeId")
            order.set("productInstrumentType","Equity")
            order.set("isEnhancedLimitOrder",True)
            #order.set("traderExchangeId","clamch")
        ...
        if self._data["vikingMarketId"] == "SYDE":
            ack = self.api_.enqueue_message("TESTC->%sASXA" % settings.VIKING_ENV ,"VikingOrder",order.msg)
        elif self._data["vikingMarketId"] == "CHIA":
            ack = self.api_.enqueue_message("TESTA->%sCXAA" % settings.VIKING_ENV ,"VikingOrder",order.msg)
        else:
        ...
        if validate:
            self._events[orderId].wait(settings.VIKING_ACK_WAIT)

            ret = False
            ## check order ack
            if len(self.acks) ==0 :
                raise ValueError("No VikingOrder ack in %d seconds, order data: %s" % (settings.VIKING_ACK_WAIT,self._data))
    ...
    def amend(self,**kw):
        ...
        ## SYDE or CXA order
        if self._data["vikingMarketId"] == "SYDE":
            ack = self.api_.enqueue_message("TESTC->%sASXA" % settings.VIKING_ENV ,"VikingAmendOrder",order.msg)
        elif self._data["vikingMarketId"] == "CHIA":
            ack = self.api_.enqueue_message("TESTA->%sCXAA" % settings.VIKING_ENV ,"VikingAmendOrder",order.msg)
        else:
            raise ValueError("unsupported exchange: %s, only support (SYDE/CHIA)" % self._data["vikingMarketId"])
    ...
    def cancel(self):
        ...
        if len(self.acks) > 0:
            ...
            ## SYDE or CXA order
            if self._data["vikingMarketId"] == "SYDE":
                ack = self.api_.enqueue_message("TESTC->%sASXA" % settings.VIKING_ENV ,"VikingCancelOrder",order.msg)
            else:
                ack = self.api_.enqueue_message("TESTA->%sCXAA" % settings.VIKING_ENV ,"VikingCancelOrder",order.msg)
    ...
    def expect(self,expect, **kw):
        ...
        timeout = kw.get("timeout",settings.VIKING_ACK_WAIT)
        wait    = kw.get("wait")
...
## clean up dss on exit
#atexit.register(VikingOrder.tracker_.shutdown)
...
def run_unittest():
    ...
    for i in range(3):
        try:
            ...
            order.new(symbol="NAB",price=22.4,qty=109)
        except IndexError:
            pass
    #exch="CHIA"
    order = VikingOrder(exch=exch)
